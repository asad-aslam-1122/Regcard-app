...
import 'package:regcard/src/base/view/home/view/widgets/chat_options_sheet.dart';
...
class ConversationView extends StatefulWidget {
  static String route = "/ConversationView";
...
class _ConversationViewState extends State<ConversationView> {
  ...
  Future<void> onLoading(HomeVm vm) async {
    if ((vm.chatModel.totalRecords ?? 0) >
        (vm.chatModel.message ?? []).length) {
      pageNumber += 1;
      await vm.getChatById(
                ...
                children: [
                  Expanded(
                    child: RefreshIndicator(
                        ...
                        onRefresh: () => onLoading(homeVm),
                        child: ListView.builder(
                          controller: homeVm.controller,
                          itemCount: homeVm.chatModel.message?.length ?? 0,
                          itemBuilder: (context, index) {
                            return Column(
                              children: [
                                _getGroupSeparator(
                                    homeVm.chatModel.message ?? [], index),
                                // homeVm.otherUser?.id ==
                                //         AppUser.userProfile?.id
                                homeVm.chatModel.message?[index].sender?.id ==
                                        AppUser.userProfile?.id
                                    ? sendMsg(
                                        (homeVm.chatModel.message ?? [])[index],
                                        index)
    ...
    return Align(
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            DisplayImage(
              imageUrl: msg.sender?.pictureUrl ?? "",
              borderColor: R.colors.lightGreyColor,
              borderWidth: 0,
              isCircle: true,
              hasMargin: false,
